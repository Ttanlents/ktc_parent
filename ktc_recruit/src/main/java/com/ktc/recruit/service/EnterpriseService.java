package com.ktc.recruit.service;

import com.ktc.recruit.dao.EnterpriseDao;
		...
		return new Specification<Enterprise>() {
			...
			public Predicate toPredicate(Root<Enterprise> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (searchMap.get("jobcount")!=null && !"".equals(searchMap.get("jobcount"))) {
					predicateList.add(cb.like(root.get("jobcount").as(String.class), "%"+(String)searchMap.get("jobcount")+"%"));
				}
				//URL
